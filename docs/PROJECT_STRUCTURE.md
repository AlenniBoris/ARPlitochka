...
- **`:app`**: Android-приложение (Koin, MainActivity, навигация).
...
3. **DI**: Используется **Koin** для всех платформ. Инициализация происходит в `shared:app` через `initKoin()`. Платформенные модули (`:app` и `:iosApp`) вызывают эту функцию при старте.
