# iOS: Непрерывная Поверхность Пола — План Placement Mode

> Исторический implementation-plan по iOS placement mode. Канонические правила surface detection находятся в [`AR/SURFACE_DETECTION.md`](../../ar/PLATFORM_SURFACE_DETECTION.md), стабильность контура — в [`AR/CONTOUR_STABILITY.md`](../../ar/PLATFORM_CONTOUR_STABILITY.md).
...
- [x] Placement patch — только fallback, не основной путь
- [x] Debug: `Tap frame age`, защита тапа при stale frame / большом `Camera gap`

### Фаза E — Гибридный Placement (Референс + Исследование)

- [x] Референсная сетка: одна frozen overlay (`continuous-floor`)
...
- [x] Референс + exploration (см. фазу E)
...
### Фаза 6 — Замыкание и заливка (Android parity)

- [x] Быстрое замыкание через render-driven snap (в обход throttled onFrame)
- [x] Синяя полупрозрачная заливка (ear-clipping/fan) сразу при `Closed: Yes`
- [x] Замыкающее ребро сохраняется после финализации
- [x] Зелёные точки остаются видимыми после финализации (UX parity)
- [x] Anchor-correction и manual realign работают после финализации
- [x] Render-loop sync контура при delegate starvation (gap ~10s)
- [x] Debug: поле `Finalized`
...
14. При `Closed: Yes` — сразу видны **зелёные точки, синие линии и синяя заливка**
15. После галочки — точки **остаются**, заливка и замыкающее ребро не пропадают
16. После фриза контур не «открывается» (latch closed)
...
*Фазы A–E, фиксы 0–4 и фаза 5 (patch + point stability) реализованы в коде. Delegate starvation — отдельно: [ios-ar-delegate-starvation-handoff-prompt.md](../../investigations/ios-ar-delegate-starvation-handoff-prompt.md).*
