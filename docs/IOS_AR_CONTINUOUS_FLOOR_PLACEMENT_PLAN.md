...
**Статус:** в работе (фазы A–E + фиксы 0–4 реализованы, ожидают прогон на устройстве)  
**Дата:** июнь 2026  
**Контекст:** обсуждение после тестов на устройстве — placement patch под прицелом отстаёт на 2–3 секунды; предложение вернуться к scan-плоскости на всём этапе контура.
...
- Точки и сетка могут «плыть» при relocalization (частично закрыто ARAnchor-корнем контура)
...
| Точки плывут | ARAnchor-корень контура (уже реализован) |
...
*Фазы A–E и фиксы 0–4 реализованы в коде; требуется прогон на iPhone.*
