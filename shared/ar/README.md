...
В текущем приложении этот модуль используется сценарием разметки пола: он помогает выбрать пригодные горизонтальные плоскости и получить hit-test из центра экрана для reticle.
...
Эта функция не создает пользовательские точки и не управляет anchors. Она только возвращает подходящий `HitResult`; решение о создании `Anchor` принимает `FloorArViewModel` в feature-модуле.
...
- общие проверки tracking/plane состояния.
...
- выбор текстур плитки;
- SceneView-визуализацию конкретной фичи.
